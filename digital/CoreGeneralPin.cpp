 ...
 * Using
 */
using core::digital::CoreGeneralPin;
 ...
 *
 */
CoreGeneralPin::CoreGeneralPin(CoreGeneralPort &base, uint32_t pin) : mReg(base.getRegister()) {
  this->mPin = pin;
}
...
bool CoreGeneralPin::getDir(void) {
  if (this->mPin >= 8)
    return GET_CTRL_DIR(mReg.cfglr, this->mPin);
  else
    return GET_CTRL_DIR(mReg.cfghr, (this->mPin - 8));
}
  ...
  switch (mode) {
    ...
    case GeneralPinMode::PUSH_PULL:
      ...
      *reg = (ctrl | static_cast<uint32_t>(0x00000003 << shift));
      return GeneralPinMode::PUSH_PULL;
    ...
    case GeneralPinMode::PULL_UP:
      ...
      *reg = (ctrl | static_cast<uint32_t>(0x00000008 << shift));
      this->mReg.scr |= (1 << this->mPin);
      return GeneralPinMode::PULL_UP;
    ...
    case GeneralPinMode::PULL_DOWN:
      ...
      *reg = (ctrl | static_cast<uint32_t>(0x00000008 << shift));
      this->mReg.clr |= (1 << this->mPin);
      return GeneralPinMode::PULL_DOWN;
    ...
    case GeneralPinMode::OPEN_DRAIN:
      ...
      return GeneralPinMode::OPEN_DRAIN;
  }
...
bool CoreGeneralPin::getValue(void) {
  return (this->mReg.idt & (1 << this->mPin));
}
