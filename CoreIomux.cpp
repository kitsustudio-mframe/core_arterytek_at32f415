...
namespace core {
#pragma pack(4)
  static uint8_t mCoreIomuxMemory[sizeof(CoreIomux)];
#pragma pack()
}  // namespace core
...
CoreIomux* CoreIomux::mInstance = nullptr;
...
//-----------------------------------------------------------------------------
void CoreIomux::remapEXINT(MapEXINT map, uint8_t pin) {
  if (pin >= 16)
    return;
 ...
 */
//-----------------------------------------------------------------------------
void CoreIomux::instantiation(void) {
  if (CoreIomux::mInstance)
    return;

  CoreIomux::mInstance = new (&core::mCoreIomuxMemory) CoreIomux();
  return;
}
